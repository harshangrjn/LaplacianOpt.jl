...
function constraint_lazycallback_wrapper(lom::LaplacianOptModel;
    ...
    function constraint_lazycallback_cuts(cb_cuts)
        ...
        elseif status in [MOI.CALLBACK_NODE_STATUS_INTEGER, MOI.CALLBACK_NODE_STATUS_FRACTIONAL]
    ...
    JuMP.set_attribute(lom.model, MOI.LazyConstraintCallback(), constraint_lazycallback_cuts)
end
...
function constraint_cheeger_cuts(
    ...
    result_dict = LOpt.cheeger_constant(z_val .* lom.data["adjacency_augment_graph"], optimizer)

    ac_lower_bound = floor(lom.options.best_lower_bound, digits = 3)

    if !(isapprox(result_dict["cheeger_constant"], 0, atol = 1E-5)) && (result_dict["cheeger_constant"] < (ac_lower_bound * lom.options.cheeger_cuts_factor))
        lom.result["cheeger_cuts_separation_time"] += result_dict["solve_time"]
        S = result_dict["S"]
        S_complement = result_dict["S_complement"]

        con = JuMP.@build_constraint(
            sum(lom.data["adjacency_augment_graph"][i,j] * lom.variables[:z_var][i,j] for i in S, j in S_complement)
